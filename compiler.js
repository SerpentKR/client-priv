...
function recursive(dir = __dirname) {
    ...
    for (let item of dirListing) {
        ...
        } else if (item.isFile() && item.name.endsWith('.js') && path.join(dir, item.name) !== __filename && item.name != "client.js" /*&& item.name != "preload.js" && item.name != "splashPreload.js"*/) {
            let filePath = path.join(dir, item.name);
            console.log(`Compiling ${filePath}`);
            bytenode.compileFile(filePath, filePath + 'c');
            fs.unlinkSync(filePath);
            // Allows preloader to be used in bytenode compile.
